...
const QString Sample::status_names[STATUS_NUM] = {"UNKNOWN", "ROLLING", "CHIPPED", "FLYING", "KICKED", "BOUNCED", "LYING"};
const QString Sample::last_touched_names[LAST_TOUCHED_NUM] = {"UNKNOWN", "YELLOW", "BLUE", "REFEREE"};
...
Ball_Sample::Ball_Sample(): Sample(), pos(0.,0.,0.), speed(0.,0.,0.), last_touched_robot(-1, -1), ball_line(0., 0., 0., 0.), polarbaer(0.,0.)
{
...
Ball_Sample::Ball_Sample(BSmart::Pose3D pos_, BSmart::Pose3D speed_, Status status_, Last_Touched last_touched_)
                       : Sample(), pos(pos_), speed(speed_), last_touched_robot(-1, -1), ball_line(0., 0., 0., 0.), polarbaer(0.,0.)
{
    status = status_;
    weighting = 0.0000001;
    type = BALL;
...
Ball_Sample::Ball_Sample(const Ball_Sample& other): Sample(), ball_line(0., 0., 0., 0.), polarbaer(0.,0.)
{
    status = other.status;
...
void Ball_Sample::move(const double ms, const Robot_Sample_List& robot_obstacles)
{
    switch (status)
    {
        case KICKED:
...
void Ball_Sample::check_collisions(const Robot_Sample_List& robot_obstacles, double ms)
{
    ...
    do
    {
        ...
        intersect = false;
        //check for collision
        intersect = intersect || check_robot_reflections(hitpoint, robot_obstacles);
        ...
        if(intersect)
        {
            ...
            const double dist_to_collision = last_pos.distance_to_2D(collision3D);
...
//                        std::cout << "collision without obstacle - please check" << std::endl;
    ...
    //                    if(cnt==0)
                        {
                            BSmart::Double_Vector normal(hitpoint->collisionpoint - hitpoint->orientation_point);
                            last_pos = collision3D;
                            ...
                            switch (status)
                            {
                                ...
                                default:
                                    ...
                                    const double dist_after_collision = dist_total_2D - dist_to_collision;
...
//        if(test_intersection_2(ball_line, Field_Hardware::field_bars_ball[i].line))
        {
            if(test_intersection(ball_line, Field_Hardware::field_bars_ball[i].line))
            {
                if(intersection_point(ball_line, Field_Hardware::field_bars_ball[i].line, &intersection))
                {
                    //reflection?
                    if( Field_Hardware::field_bars_ball[i].point_a.distance_to(intersection) < Field_Hardware::field_bars_ball[i].dist &&        //2d
                        Field_Hardware::field_bars_ball[i].point_b.distance_to(intersection) < Field_Hardware::field_bars_ball[i].dist &&        //2d
                        last_pos.distance_to_2D(intersection) < last_pos.distance_to_2D(pos) &&//2d
                        pos.distance_to_2D(intersection) < last_pos.distance_to_2D(pos) )      //2d
                    {
                        if(last_pos.distance_to_2D(intersection) < hitpoint->dist)             //2d
                        {
                            hitpoint->dist = last_pos.distance_to_2D(intersection);            //2d
                            hitpoint->collisionpoint = intersection;
                            hitpoint->obstacle = Field_Hardware::field_bars_ball[i].obstacle;
                            hitpoint->orientation_point = Field_Hardware::field_bars_ball[i].point_a;
                            hitpoint->height = Field_Hardware::field_bars_ball[i].height;
                            return true;
                        }
                    }
                }
...
//        double z_div = abs(pos.z - last_pos.z);
        ...
        {
            hitpoint->dist = dist;
            hitpoint->collisionpoint = BSmart::Double_Vector(last_pos.x + quot*(pos.x-last_pos.x), last_pos.y + quot*(pos.y-last_pos.y));
...
bool Ball_Sample::check_goalpost_reflections(Hitpoint* hitpoint)
{
    ...
    {
        if(test_intersection(Field_Hardware::field_goalposts_ball[i].circle, ball_line))
        {
            if(intersection_point(Field_Hardware::field_goalposts_ball[i].circle, ball_line, intersections))
            {
...
//                    std::cout << "intersections["<<j<<"]: " << intersections[j] << std::endl;
                    if(pos.distance_to_2D(intersections[j]) < last_pos.distance_to_2D(pos) &&
                       last_pos.distance_to_2D(intersections[j]) < last_pos.distance_to_2D(pos))
                    {
                        if(last_pos.distance_to_2D(intersections[j]) < hitpoint->dist)
                        {
                            hitpoint->dist = last_pos.distance_to_2D(intersections[j]);
                            hitpoint->collisionpoint = intersections[j];
                            hitpoint->obstacle = Field_Hardware::field_goalposts_ball[i].obstacle;
                            hitpoint->orientation_point = Field_Hardware::field_goalposts_ball[i].center;
                            hitpoint->height = Field_Hardware::field_goalposts_ball[i].height;
                            intersect = true;
                        }
                    }
                }
...
bool Ball_Sample::check_robot_reflections(Hitpoint* hitpoint, const Robot_Sample_List& robot_obstacles)
{
    ball_line.p1.x = last_pos.x;
    ball_line.p1.y = last_pos.y;
    ball_line.p2.x = pos.x;                                              //2d
    ...
    const Robot_Sample_List::const_iterator it_obstacles_end = robot_obstacles.end();
    ...
    while(it_obstacles != it_obstacles_end)
    {
        ...
        else if (pos.distance_to_2D(it_obstacles->pos) < (BSmart::Field::ball_radius + BSmart::Field::robot_radius + 40) )
        {
            if(pos.distance_to_2D(it_obstacles->pos) < hitpoint->dist)
            {
                if(random < 0.3)//30% Chance zu kollidieren wenn er nah dran vorbei rollt oder nah dran liegt
                {
                    ...
                    hitpoint->collisionpoint = BSmart::Double_Vector(pos.x, pos.y);
                    if(it_obstacles->team)
                        hitpoint->obstacle = Field_Hardware::ROBOT_BLUE;
                    else
                        hitpoint->obstacle = Field_Hardware::ROBOT_YELLOW;
...
Robot_Sample::Robot_Sample(): pos(0.,0.,0.), speed(0.,0.,0.), polarbaer(0.,0.)
{
    weighting = 0.0000001;
    status = STATUS_UNKNOWN;
    type = ROBOT;
    timestamp = 0.;    double factor = 0.5;
    //factor should be between 0.5 and 1.5, 10 m/s maximum speed
    factor += 0.1 * speed.length();
...
Robot_Sample::Robot_Sample(const Robot_Sample& other): polarbaer(0.,0.)
{
    pos = other.pos;
    speed = other.speed;
...
void Robot_Sample::move(const double ms, const Robot_Sample_List& robot_obstacles)
{
    last_pos = pos;
    pos += (speed * ms);
        ...
        if(intersect)
        {
            ...
            switch(hitpoint->obstacle)
            {
                case Field_Hardware::NO_OBSTACLE:
                    ...
                    break;
                case Field_Hardware::BAR_HORIZONTAL:
                    pos.y = hitpoint->collisionpoint.y + ( hitpoint->collisionpoint.y - pos.y );
                    last_pos = hitpoint->collisionpoint;
                    speed.y *= -1;//-0.8;
                    break;
                case Field_Hardware::BAR_VERTICAL:
                    pos.x = hitpoint->collisionpoint.x + ( hitpoint->collisionpoint.x - pos.x );
                    last_pos = hitpoint->collisionpoint;
                    speed.x *= -1;//-0.8;
                    break;
                case Field_Hardware::FLOOR:
                    std::cout << "sample.cc Robot_Sample::check_collisions FLOOR: This should not happen." << std::endl;
                    break;
                case Field_Hardware::GOALPOST:
                case Field_Hardware::ROBOT_BLUE:
                case Field_Hardware::ROBOT_YELLOW:
...
//        if(test_intersection_2(robot_line, Field_Hardware::field_bars_robot[i].line))
        {
            if(test_intersection(robot_line, Field_Hardware::field_bars_robot[i].line))
            {
                if(intersection_point(robot_line, Field_Hardware::field_bars_robot[i].line, &intersection))
                {
                    //reflection?
                    if( Field_Hardware::field_bars_robot[i].point_a.distance_to(intersection) < Field_Hardware::field_bars_robot[i].dist &&
                        Field_Hardware::field_bars_robot[i].point_b.distance_to(intersection) < Field_Hardware::field_bars_robot[i].dist &&
                        last_pos.distance_to(intersection) < last_pos.distance_to(pos) &&
                        pos.distance_to(intersection) < last_pos.distance_to(pos))
                    {
                        if(last_pos.distance_to(intersection) < hitpoint->dist)
                        {
                            hitpoint->dist = last_pos.distance_to(intersection);
                            hitpoint->collisionpoint = intersection;
                            hitpoint->obstacle = Field_Hardware::field_bars_robot[i].obstacle;
                            hitpoint->orientation_point = Field_Hardware::field_bars_robot[i].point_a;
                            return true;
                        }
                    }
                }
...
bool Robot_Sample::check_goalpost_reflections(Hitpoint* hitpoint)
{
    ...
    for(int i = 0; i < Field_Hardware::NUMBER_FIELD_GOALPOSTS; ++i)
    {
        if(test_intersection(Field_Hardware::field_goalposts_robot[i].circle, robot_line))
        {
            if(BSmart::intersection_point(Field_Hardware::field_goalposts_robot[i].circle, robot_line, intersections))
            {
//                std::cout << "intersections.size(): " << intersections.size() << std::endl;
                for(unsigned int j = 0; j < intersections.size(); ++j)
                {
//                    std::cout << "intersections["<<j<<"]: " << intersections[j] << std::endl;
                    if(pos.distance_to(intersections[j]) < last_pos.distance_to(pos) &&
                       last_pos.distance_to(intersections[j]) < last_pos.distance_to(pos))
                    {
                        if(last_pos.distance_to(intersections[j]) < hitpoint->dist)
                        {
                            hitpoint->dist = last_pos.distance_to(intersections[j]);
                            hitpoint->collisionpoint = intersections[j];
                            hitpoint->obstacle = Field_Hardware::field_goalposts_robot[i].obstacle;
                            hitpoint->orientation_point = Field_Hardware::field_goalposts_robot[i].center;
                            return true;
                        }
                    }
                }
...
bool Robot_Sample::check_robot_reflections(Hitpoint* hitpoint, const Robot_Sample_List& robot_obstacles)
{
    robot_line.p1 = last_pos;
    robot_line.p2 = pos;
    intersections.clear();
...
//    int num_bots = robot_obstacles.size();
    ...
    while(it_obstacles != it_obstacles_end)
    {
        if( !( (it_obstacles->team == this->team) && (it_obstacles->id == this->id)))
        {
