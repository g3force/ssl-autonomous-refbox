...
Pre_Filter_Data::Pre_Filter_Data()
{
    ...
    //cam 0 left
    camera_pos[0].cam_pos = BSmart::Pose3D(-(BSmart::Field::half_field_width/2), 0., 4000.);
    camera_pos[1].cam_pos = BSmart::Pose3D(  BSmart::Field::half_field_width/2 , 0., 4000.);
...
void Pre_Filter_Data::set_ball_timestamp(int camID, const BSmart::Time_Value& timestamp)
{
    pf_data_mutex.lock();
        if(current_balls[camID].size() > 0)
...
void Pre_Filter_Data::set_robots(int camID, int team, int id, const Robot_Percept_List& pRobots)
{
    pf_data_mutex.lock();
...
void Pre_Filter_Data::set_camera_pos(int camID, const BSmart::Pose3D& new_pos)
{
    pf_data_mutex.lock();
        ...
        {
            camera_pos[camID].cam_pos.x += (new_pos.x - camera_pos[camID].cam_pos.x)/2;
            camera_pos[camID].cam_pos.y += (new_pos.y - camera_pos[camID].cam_pos.y)/2;
            camera_pos[camID].cam_pos.z += (new_pos.z - camera_pos[camID].cam_pos.z)/2;
...
void Pre_Filter_Data::set_ball_direction_before(const BSmart::Pose& ball_direction_before_)
{
    pf_data_mutex.lock();
...
void Pre_Filter_Data::set_ball_direction_after(const BSmart::Pose& ball_direction_after_)
{
    pf_data_mutex.lock();
...
void Pre_Filter_Data::set_robot_direction(int team, int id, const BSmart::Pose& robot_direction_)
{
    pf_data_mutex.lock();
