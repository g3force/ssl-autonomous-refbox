...
%Distance for different things like Free Kick and other constants
%@param OpponentsBeforeKickOff - Law 8 - 500mm
%@param BallOtOufPlay - Law 9 - 500mm
%@param OpponentAtDefArea - Law 9 - 200mm
%@param BallNotEnterInTime - Law 9 - 10s
%@param BallNotEnterDist - Law 9 - 500mm
%@param DribblingTooMuch - Law 12 - 500mm
%@param BallTooHighIntoGoal - Law 12 - 150mm - how too check?
%@param BallSpeed - Law 12 - 8m/s
%@param FreeKickInDefAreaFromGoal - Law 13 - 600mm
%@param FreeKickInDefAreaFromTouch - Law 13 - 100mm
%@param FreeKickInDefAreaAttack - Law 13 - 700m
%@param FreeKickOtherRob - Law 13 - 500m
%@param PenaltyKickOtherRob - Law 14 - 400mm
%@param ThrowIn - Law 15 - 100mm
%@param ThrowInOtherRob - Law 15 - 500mm
%@param GoalKickFromLine - Law 16 - 500mm
%@param GoalKickFromTouch - Law 16 - 100mm
%@param GoalKickOppRob - Law 16 - 500mm
%@param CornerKick - Law 17 - 100mm
%@param CornerKickOppRob - Law 17 - 500mm

:- dynamic constants/21.
define_constants(OpponentsBeforeKickOff,BallOtOufPlay, OpponentAtDefArea, BallNotEnterInTime, BallNotEnterDist, DribblingTooMuch, BallTooHighIntoGoal,
		BallSpeed, FreeKickInDefAreaFromGoal, FreeKickInDefAreaFromTouch, FreeKickInDefAreaAttack, FreeKickOtherRob, PenaltyKickOtherRob, ThrowIn,
		ThrowInOtherRob, GoalKickFromLine, GoalKickFromTouch, GoalKickOppRob, CornerKick, CornerKickOppRob) :- 
	not(constants('constants',_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_)) , 
	assert(constants('constants',
		OpponentsBeforeKickOff,	BallOtOufPlay, OpponentAtDefArea, BallNotEnterInTime, BallNotEnterDist, DribblingTooMuch, BallTooHighIntoGoal,
		BallSpeed, FreeKickInDefAreaFromGoal, FreeKickInDefAreaFromTouch, FreeKickInDefAreaAttack, FreeKickOtherRob, PenaltyKickOtherRob, ThrowIn,
		ThrowInOtherRob, GoalKickFromLine, GoalKickFromTouch, GoalKickOppRob, CornerKick, CornerKickOppRob)).
...
start_game :- 
	get_local_play_state(1) , 
	get_ball_status(_,_,6,_) , 
	not(rule_sixteen) , 
	get_local_next_play_state(Next) , 
	((Next =:= 7) ; (Next =:= 8) ; (Next =:= 13) ; (Next =:= 14)) , 
	set_local_play_state(Next) , 
	Next_after is (Next-4) , 
	set_local_next_play_state(Next_after) , 
	reset_touch , 
	set_three_seconds.
...
start_game :- 
	...
	get_ball_status(_,_,6,_) , 
	not(rule_sixteen) , 
	get_local_next_play_state(2) , 
	set_local_play_state(2).
%aus before in kickoff/penalty
start_game :- 
	get_local_play_state(7) , 
	get_ball_status(_,_,6,_) , 
	not(rule_fourteen) , 
	get_timestamp(T1) ,
	get_three_seconds(T2) , 
	T3 is T1-T2 , T3 >= 3000 , 
	set_local_play_state(3) , 
	set_local_next_play_state(7) , 
	reset_touch , 
	set_ten_seconds.
start_game :- 
	get_local_play_state(8) , 
	get_ball_status(_,_,6,_) , 
	not(rule_seventeen) , 
	get_timestamp(T1) , 
	get_three_seconds(T2) , 
	T3 is T1-T2 , 
	T3 >= 3000 , 
	set_local_play_state(4) , 
	set_local_next_play_state(8) , 
	reset_touch , 
	set_ten_seconds.
start_game :- 
	get_local_play_state(13) , 
	get_ball_status(_,_,6,_) , 
	not(rule_fourteen) , 
	get_timestamp(T1) , 
	get_three_seconds(T2) , 
	T3 is T1-T2 , 
	T3 >= 3000 , 
	set_local_play_state(9) , 
	set_local_next_play_state(13) , 
	reset_touch ,  
	set_ten_seconds.
start_game :- 
	get_local_play_state(14) , 
	get_ball_status(_,_,6,_) , 
	not(rule_seventeen) , 
	get_timestamp(T1) , 
	get_three_seconds(T2) , 
	T3 is T1-T2 , 
	T3 >= 3000 , 
	set_local_play_state(10) , 
	set_local_next_play_state(14) , 
	reset_touch , 
	set_ten_seconds.
...
%right team touches
set_running :- 
	get_local_play_state(Now) , 
	ball_status('ball',Ltt,_,_,Touch) , 
	( ((Now =:= (3+(Ltt*6)) ; Now =:= (4+(Ltt*6)) ; 
		Now =:= (5+(Ltt*6)) ) , Touch >= 1) ; (Now =:= (6+(Ltt*6)) , Touch >= 0 ) ) , 
	set_local_play_state(2) , 
	set_real_play_time_start , 
	reset_ten_seconds.
%wrong team touches
set_running :- 
	get_local_play_state(Now) , 
	ball_status('ball',Ltt,_,_,Touch) , 
	( (( Now =:= (3+(abs(Ltt-1)*6)) ; Now =:= (4+(abs(Ltt-1)*6)) ; Now =:= (5+(abs(Ltt-1)*6)) ) , Touch >= 1 ) ; 
		(Now =:= (6+(abs(Ltt-1)*6)) , Touch >= 0 )) , 
	set_local_play_state(1) , 
	set_local_next_play_state(Now) , 
	reset_ten_seconds.

set_global_play_state(Play_state) :- 
	not(play_state('global_play_state',_)), 
	assert(play_state('global_play_state',Play_state)) , 
	determine_sides(Play_state) , 
	!.
set_global_play_state(Play_state) :- 
	retract(play_state('global_play_state',_)), 
	assert(play_state('global_play_state',Play_state)) , 
	determine_sides(Play_state).
get_global_play_state(Play_state) :- 
	play_state('global_play_state',Play_state).

%Falls ein kickoff und Zeit noch nicht angefangen starte Spielzeit
set_local_play_state_from_outside(Play_state) :- 
	(Play_state =:= 3 ; Play_state =:= 9) , 
	timestamp('total_play_time_start',0) , 
	set_total_play_time_start , 
	set_local_play_state(Play_state) , 
	reset_touch , 
	set_ten_seconds.
%Falls ein before_kickoff den Play_State danach setzen
set_local_play_state_from_outside(Play_state) :- 
	(Play_state =:= 7 ; Play_state =:= 8 ; Play_state =:= 13 ; Play_state =:= 14) ,  
	Next is (Play_state-4) , 
	set_local_play_state(Play_state) , 
	reset_touch , 
	set_three_seconds , 
	set_local_next_play_state(Next).
%Wenn Wechsel in running oder Wechsel aus running dann reale spielzeit starten/beenden
set_local_play_state_from_outside(New) :- 
	get_local_play_state(Old) , 
	Old =\= 2 , 
	New =:= 2 , 
	set_real_play_time_start , 
	set_local_play_state(New) , 
	reset_touch.
set_local_play_state_from_outside(New) :- 
	get_local_play_state(Old) , 
	Old =:= 2 , 
...
set_local_play_state_from_outside(Play_state) :- 
	((Play_state =:= 3) ; (Play_state =:= 4) ; (Play_state =:= 5) ; (Play_state =:= 6) ; 
	(Play_state =:= 9) ; (Play_state =:= 10) ; (Play_state =:= 11) ; (Play_state =:= 12)) , 
	set_local_play_state(Play_state) , 
	reset_touch , 
	set_ten_seconds.
%Jeder andere Fall
...
set_local_play_state(Play_state) :- 
	not(play_state('local_play_state',_)), 
	assert(play_state('local_play_state',Play_state)) , 
	reset_touch , 
	!.
set_local_play_state(Play_state) :- 
	retract(play_state('local_play_state',_)), 
	assert(play_state('local_play_state',Play_state)) , 
	reset_touch.
get_local_play_state(Play_state) :- 
	play_state('local_play_state',Play_state).
set_local_next_play_state(Play_state) :- 
	not(play_state('local_next_play_state',_)), 
	assert(play_state('local_next_play_state',Play_state)) , 
	!.
set_local_next_play_state(Play_state) :- 
	retract(play_state('local_next_play_state',_)), 
	assert(play_state('local_next_play_state',Play_state)).
get_local_next_play_state(Play_state) :- 
	play_state('local_next_play_state',Play_state).

determine_sides(Play_state) :- 
	(Play_state =:= 7 ; Play_state =:= 13) , 
	team_left(Team) , 
	set_left(Team) , 
	find_goalies.
team_left(Team) :- 
	roboter(Team,_,Pos_x,_,_,_,1) , 
	Pos_x < 0 .
set_left(Team) :- 
	not(left_team('left_team',_)) , 
	assert(left_team('left_team',Team)) , !.
set_left(Team) :- 
	retract(left_team('left_team',_)) , 
	assert(left_team('left_team',Team)).
get_left(Team) :- 
	left_team('left_team',Team).
find_goalies :- 
	find_left_goalie , 
	find_right_goalie.
find_left_goalie :- 
	field('field',Field_width,_,_,_,_,_,_,_) , 
	left_team('left_team',Left_team) , 
	roboter(Left_team,Id1,Pos_x1,Pos_y1,_,_,1) , 
	distance(Pos_x1,Pos_y1,-(Field_width/2),0,Dist1) , 
	not((roboter(Left_team,Id2,Pos_x2,Pos_y2,_,_,1) , 
	Id1 =\= Id2 , 
	distance(Pos_x2,Pos_y2,-(Field_width/2),0,Dist2) , Dist2 < Dist1)) , 
	set_left_goalie(Left_team,Id1).
find_right_goalie :- 
	field('field',Field_width,_,_,_,_,_,_,_) , 
	left_team('left_team',Left_team) , 
	roboter(Team,Id1,Pos_x1,Pos_y1,_,_,1) , 
	Team =\= Left_team , 
	distance(Pos_x1,Pos_y1,(Field_width/2),0,Dist1) , 
	not((roboter(Team,Id2,Pos_x2,Pos_y2,_,_,1) , 
	Id1 =\= Id2 , 
	distance(Pos_x2,Pos_y2,(Field_width/2),0,Dist2) , Dist2 < Dist1)) , 
	set_right_goalie(Team,Id1).
set_left_goalie(Team, Id) :- 
	not(goalie('left_goalie',_,_)) ,
	assert(goalie('left_goalie',Team,Id)) , 
	!.
set_left_goalie(Team, Id) :- 
	retract(goalie('left_goalie',_,_)) , 
	assert(goalie('left_goalie',Team,Id)).
set_right_goalie(Team, Id) :- 
	not(goalie('right_goalie',_,_)) , 
	assert(goalie('right_goalie',Team,Id)) , 
	!.
set_right_goalie(Team, Id) :- 
	retract(goalie('right_goalie',_,_)) ,
	assert(goalie('right_goalie',Team,Id)).

%testing and helping
distance(X1,Y1,X2,Y2,Dist) :- 
	Dist is sqrt((X2-X1)*(X2-X1)+(Y2-Y1)*(Y2-Y1)).
distance_to_left_def_area(X1,Y1,Dist) :- 
	field('field',Field_width,_,_,_,_,Defense_radius,Defense_line,_) ,
	...
	!.
distance_to_right_def_area(X1,Y1,Dist) :- 
	field('field',Field_width,_,_,_,_,Defense_radius,Defense_line,_) ,
	( (Y1 =< -(Defense_line/2) , distance(X1,Y1,(Field_width/2),-(Defense_line/2),Dist_tmp)) ;
		(Y1 =< (Defense_line/2) , Y1 >= -(Defense_line/2) , Dist_tmp is abs(X1-(Field_width/2))) ;
		(Y1 >= (Defense_line/2) , distance(X1,Y1,(Field_width/2),(Defense_line/2),Dist_tmp) ) ) ,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rule_zero :- 
	get_global_play_state(X) , 
	(get_local_play_state(X) ; get_local_play_state(2)).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Regel30: Ball im Spielfeld, nur im running										%%
%% coresspond to law 9: The Ball In and Out of Play									%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ball_in_game(Field_width,Field_height,Pos_x,Pos_y) :- 
	Pos_x > -(Field_width/2) , 
	Pos_x < Field_width/2 , 
...
rule_thirty_before :- 
	get_local_play_state(2).

rule_thirty :- 
	rule_thirty_before , 
	field('field',Field_width,Field_height,_,_,_,_,_,_) , 
	ball_location('ball',Pos_x,Pos_y,_,_,_,_) , 
	not(ball_in_game(Field_width,Field_height,Pos_x,Pos_y)) , 
	rule_thirty_after.

rule_thirty_after :- 
	(set_freekick_pos_throw_in ; set_freekick_pos_goal_out) , ball_status('ball',Ltt,Ltid,_,_) , 
	set_rule_breaker(Ltt,Ltid).
set_freekick_pos_throw_in :- 
	ball_location('ball',Pos_x,Pos_y,_,_,_,_) , 
	ball_status('ball',Last_touched_team,_,_,_) , 
	(throw_in_above(Pos_x,Pos_y) ; throw_in_below(Pos_x,Pos_y)) , 
	Next is (12-(Last_touched_team*6)) , 
	set_next_play_state(Next).
throw_in_above(Pos_x,Pos_y) :- 
	field('field',_,Field_height,_,_,_,_,_,_) , 
	constants('constants',_,_,_,_,_,_,_,_,_,_,_,_,_,ThrowIn,_,_,_,_,_,_) ,
	Pos_y > Field_height/2 , 
...
set_freekick_pos_goal_out :- 
	ball_location('ball',Pos_x,Pos_y,_,_,_,_) , 
	ball_status('ball',Last_touched_team,_,_,_) , 
	(goal_out_left(Pos_x,Pos_y,Last_touched_team) ; goal_out_right(Pos_x,Pos_y,Last_touched_team)) , 
	Next is (11-(Last_touched_team*6)) , 
	set_next_play_state(Next).
%corner left above
goal_out_left(Pos_x,Pos_y,Last_touched_team) :- 
	field('field',Field_width,Field_height,_,_,_,_,_,_) , 
	constants('constants',_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,CornerKick,_) ,
	left_team('left_team',Left_team) , 
	Pos_x < -(Field_width/2) , 
...
goal_out_left(Pos_x,Pos_y,Last_touched_team) :- 
	field('field',Field_width,Field_height,_,_,_,_,_,_) , 
	constants('constants',_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,GoalKickFromLine,GoalKickFromTouch,_,_,_) ,
	left_team('left_team',Left_team) , 
	Pos_x < -(Field_width/2) , 
	(Last_touched_team =\= Left_team) , 
	Pos_y > 0 , 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rule_fifteen_before :- 
	freekick_blue ; freekick_yellow.
freekick_yellow :- 
	get_local_play_state(3).
freekick_yellow :- 
	get_local_play_state(5).
freekick_yellow :- 
	get_local_play_state(6).
freekick_yellow :- 
	get_local_play_state(7).
freekick_blue :- 
	get_local_play_state(9).
freekick_blue :- 
	get_local_play_state(11).
freekick_blue :- 
	get_local_play_state(12).
freekick_blue :- 
	get_local_play_state(13).
rule_fifteen :- 
	rule_fifteen_before , 
...
rule_fifteen_distance_to_ball :- 
	roboter(Team,ID,Pos_x_bot,Pos_y_bot,_,_,1) , 
	constants('constants',_,_,_,_,_,_,_,_,_,_,_,FreeKickOtherRob,_,_,_,_,_,_,_,_) ,
	Team =:= 0 , 
	freekick_blue , 
	ball_location('ball',Pos_x_ball,Pos_y_ball,_,_,_,_) , 
	distance(Pos_x_bot,Pos_y_bot,Pos_x_ball,Pos_y_ball,Dist) , 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rule_fourteen_before :- 
	kickoff_blue ; kickoff_yellow.
kickoff_blue :- 
	get_local_play_state(X) , 
	(X =:= 9 ; X =:= 13).
kickoff_yellow :- 
	get_local_play_state(X) , 
	(X =:= 3 ; X =:= 7).
rule_fourteen :- 
	rule_fourteen_before , 
	get_left(Left_team) , 
	roboter(Team,ID,Pos_x,_,_,_,1) , 
	( (Left_team =:= Team , Pos_x > 0) ; (Left_team =\= Team , Pos_x < 0) ) , 
	set_rule_breaker(Team,ID).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rule_three :- 
	(check_left_goalie ; check_right_goalie).
check_left_goalie :- 
	field('field',Field_width,_,_,_,_,_,_,_) , 
	get_left(Left_team) , 
	goalie('left_goalie',Left_team,Id1) , 
	roboter(Left_team,Id1,Pos_x1,Pos_y1,_,_,1) , 
	distance(Pos_x1,Pos_y1,-(Field_width/2),0,Dist1) , 
	roboter(Left_team,Id2,Pos_x2,Pos_y2,_,_,1) , 
	Id1 =\= Id2 , 
	distance(Pos_x2,Pos_y2,-(Field_width/2),0,Dist2) , 
	Dist2 < Dist1 , 
	set_rule_breaker(Left_team,Id2).
check_right_goalie :- 
	field('field',Field_width,_,_,_,_,_,_,_) , 
	get_left(Left_team) , 
	goalie('right_goalie',Team,Id1) , 
	Team =\= Left_team , 
	roboter(Team,Id1,Pos_x1,Pos_y1,_,_,1) , 
	distance(Pos_x1,Pos_y1,(Field_width/2),0,Dist1) , 
	roboter(Team,Id2,Pos_x2,Pos_y2,_,_,1) , 
	Id1 =\= Id2 , 
	distance(Pos_x2,Pos_y2,(Field_width/2),0,Dist2) , 
	Dist2 < Dist1 , 
	set_rule_breaker(Team,Id2).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ball_in_right_goal :- 
	field('field',Field_width,_,Goal_width,Goal_Depth,Goal_Height,_,_,_) , 
	ball_location('ball',Pos_x,Pos_y,Pos_z,_,_,_) , 
	Pos_x > (Field_width/2) , 
	Pos_x < ((Field_width/2)+Goal_Depth) , 
	Pos_y > -(Goal_width/2) , 
	Pos_y < (Goal_width/2) , 
	Pos_z < Goal_Height.
ball_in_left_goal :- 
	...
	Pos_z < Goal_Height.
second_touch :- 
	ball_status('ball',Ltt,Ltid,Status,X) , 
	Status =\= 3 , 
	X > 0 , 
	set_rule_breaker(Ltt,Ltid).
...
rule_twentynine :- 
	rule_twentynine_before , 
	second_touch , 
	((ball_in_right_goal , rule_twentynine_after(1)) ; 
		(ball_in_left_goal , rule_twentynine_after(0))).
rule_twentynine_after(Side) :- 
	get_left(Left_team) , 
	( (Side =:= 1 , goal(Left_team) , Next is (13-(Left_team*6)),  set_next_play_state(Next)) ; 
		( Side =:= 0,  goal(abs(Left_team-1)) , Next is (13-(abs(Left_team-1)*6)) , set_next_play_state(Next) ) ) , 
	set_freekick_pos(0,0,0).

set_goals(X,Y) :- 
	retract(result('result',_,_)) , 
...
penalty_blue_on_left_goal :- 
	...
	constants('constants',_,_,_,_,_,_,_,_,_,_,_,_,PenaltyKickOtherRob,_,_,_,_,_,_,_) ,
...
penalty_blue_on_left_goal :- 
	...
	Pos_x_other < Min_dist , 
	( ( Pos_x_kicker < Pos_x_other , set_rule_breaker(1,Other_id)) ; 
		( Pos_x_kicker > Pos_x_other , set_rule_breaker(1,Kicker_id)) ).
...
penalty_yellow_on_left_goal :- 
	...
	Pos_x_other < Min_dist , 
	( ( Pos_x_kicker < Pos_x_other , set_rule_breaker(1,Other_id)) ; 
		( Pos_x_kicker > Pos_x_other , set_rule_breaker(0,Kicker_id)) ).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rule_twentyeight :- 
	get_timeout_start_yellow(Start) , 
	Start =\= 0 , 
	get_timestamp(Timestamp) , 
	get_timeout_time_yellow(Old) , 
	New is Old+(Timestamp-Start) , 
	New > 300000.
rule_twentyeight :- 
	get_timeout_start_blue(Start) , 
	Start =\= 0 , 
	get_timestamp(Timestamp) , 
	get_timeout_time_blue(Old) , 
	New is Old+(Timestamp-Start) , 
	New > 300000.
...
rule_twentyseven :- 
	get_timeout_start_yellow(Start) , 
	Start =\= 0 , 
	timeouts('timeouts',Yellow,_) , 
	Yellow > 4.
rule_twentyseven :- 
	get_timeout_start_blue(Start) , 
	Start =\= 0 , 
	timeouts('timeouts',_,Blue) , 
	Blue > 4.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rule_twentyfour :- 
	get_total_play_time_start(Start) , 
	Start =\= 0 , 
	get_total_play_time_total(Total) , 
	get_total_play_time_start(Start) , 
	get_timestamp(Now) , 
	Sum is Total+(Now-Start) , 
	Sum > 600000.
...
rule_eighteen :- 
	get_local_play_state(2) , 
	goalie('left_goalie',Team,Id1) , 
	roboter(Team,Id2,Pos_x2,Pos_y2,_,_,1) , 
	Id1 =\= Id2 , 
	distance_to_left_def_area(Pos_x2,Pos_y2,Dist) , 
	Dist < 0 , 
	set_rule_breaker(Team,Id2).
rule_eighteen :- 
	get_local_play_state(2) , 
	...
	roboter(Team,Id2,Pos_x2,Pos_y2,_,_,1) , 
	Id1 =\= Id2 , 
	distance_to_right_def_area(Pos_x2,Pos_y2,Dist) , 
	Dist < 0 , 
	set_rule_breaker(Team,Id2).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rule_twentythree :- 
	get_local_play_state(PS) , 
	( (PS =:= 3) ; (PS =:= 4) ; (PS =:= 5) ; (PS =:= 6) ; (PS =:= 9) ; (PS =:= 10) ; (PS =:= 11) ; (PS =:= 12) ) , 
	get_ten_seconds(Ten) , 
	Ten =\= 0 , 
	get_timestamp(Time) , 
	Diff is Time-Ten , 
...
rule_nineteen :- 
	get_local_play_state(2) , 
	goalie('left_goalie',Team1,Id1) , 
	roboter(Team1,Id1,Pos_x1,Pos_y1,_,_,1) , 
	roboter(Team2,Id2,Pos_x2,Pos_y2,_,_,1) , 
	Team1 =\= Team2 , 
...
rule_twentytwo :- 
	get_local_play_state(PS) , 
	( (PS =:= 7) ; (PS =:= 8) ; (PS =:= 13) ; (PS =:= 14) ) , 
	ball_status('ball',Ltt,Ltid,_,Touch) , 
	Touch > 0 , 
	set_local_play_state(1) , 
	set_local_next_play_state(PS), 
	set_rule_breaker(Ltt,Ltid).
...
rule_fourtytwo :- 
	...
	retract(check_offside(_)) , 
	assert(check_offside(0)) , 
	rule_fourtytwo_offside.
%Left Team
rule_fourtytwo_offside :- 
	ball_status('ball',Ltt,Ltid,_,_) , 
	get_left(Ltt) , 
	roboter(Ltt,Ltid,Pos_xt,_,_,_,1) , 
	roboter(Ltt,ID1,Pos_x1,Pos_y1,_,_,1) , 
	Ltid =\= ID1 , Pos_x1 > Pos_xt , Pos_x1 > 0 , 
	not(( roboter(Team,ID2,Pos_x2,_,_,_,1) , 
	Team =\= Ltt , 
	roboter(Team,ID3,Pos_x3,_,_,_,1) , 
	ID2 =\= ID3 , Pos_x2 > Pos_x1 , Pos_x3 > Pos_x1 )) , 
	set_rule_breaker(Ltt,ID1) , 
	set_freekick_pos(Pos_x1,Pos_y1,0).
%Right Team
rule_fourtytwo_offside :- 
	ball_status('ball',Ltt,Ltid,_,_) , 
	get_left(Team) , 
