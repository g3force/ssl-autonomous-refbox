...
void PF_Tester::move_ball_respawn(int ms)
{
    ...
    //pro sekunde 5% Geschwindigkeit Verlust
...
void PF_Tester::move_ball_bounce(int ms)
{
    ...
    else if(cur_balls_0.size() > 0)
    {
        ...
        else if (new_ball.x <= (BSmart::Field::lower_own_goal_corner_back.x + BSmart::Field::ball_radius))
        {
            speed = 1. ;
        }
    }
    else if(cur_balls_1.size() > 0)
    {
        ...
        if (new_ball.x >= BSmart::Field::lower_opponent_goal_corner_back.x - BSmart::Field::ball_radius)
        {
            speed = -1.;
        }
    }
