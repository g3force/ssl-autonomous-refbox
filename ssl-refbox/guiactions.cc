...
void GuiActions::connectActions()
{
    ...
    connect(m_gui->gamearea->vision,  SIGNAL(change_record_button(QString)), this, SLOT(change_record_button(QString)));
    connect(m_gui->gamearea->vision,  SIGNAL(change_play_button(QString)), this, SLOT(change_play_button(QString)));
    ...
    connect(m_gui->log_frame_back,    SIGNAL(clicked()), m_gui->gamearea->vision->log_control, SLOT(log_frame_back()));
    connect(m_gui->log_frame_forward, SIGNAL(clicked()), m_gui->gamearea->vision->log_control, SLOT(log_frame_forward()));

    //Slider control
    connect(m_gui->gamearea->vision, SIGNAL(initializeSlider(int,int,int,int,int)), this,             SLOT(initializeSlider(int, int, int, int, int)));
//    connect(m_gui->gamearea->vision, SIGNAL(update_frame(int)), m_gui->horizontalSlider,              SLOT(setValue(int)));
    ...
    connect(m_gui->horizontalSlider, SIGNAL(sliderPressed()),   m_gui->gamearea->vision->log_control, SLOT(log_pause()));
    connect(m_gui->horizontalSlider, SIGNAL(sliderReleased()),  m_gui->gamearea->vision->log_control, SLOT(log_play()));
    connect(m_gui->gamearea,         SIGNAL(resizeSlider(int)), this,                                 SLOT(resizeSlider(int)));

    //Log Control
    connect(m_gui->gamearea->vision, SIGNAL(showLogControl(bool)), m_gui->logControl, SLOT(setVisible(bool)));
...
//    connect(m_gui->gamearea->vision, SIGNAL(update_frame(int)), m_gui->log_frameNumber, SLOT(display(int)));
    ...
    connect(m_gui->gamearea->vision->log_control, SIGNAL(update_speed(QString)), m_gui->log_speed, SLOT(setText(QString)));

    //Ball Status
    connect(m_gui->gamearea->particle_filter, SIGNAL(change_ball_status(QString)), this, SLOT(change_ball_status(QString)));
    connect(m_gui->gamearea->particle_filter, SIGNAL(change_ball_last_touched(QString)), this, SLOT(change_ball_last_touched(QString)));
