...
const std::string GLExtra::rulenames[42] =
{
    "Robot Number exceeded", //1
    "Rule not checked", //2
    "Goalie changed", //3
    "Rule not checked", //4
    "Rule not checked", //5
...
//    ball_samples = filter_data->get_ball_samples();
    ...
    for(int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team)
    {
        for(int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id )
        {
            if(filter_data->get_robot_seen(team, id))
            {
                ...
                current_robot_percepts.insert(current_robot_percepts.end(), tmp_perc_robots.begin(), tmp_perc_robots.end());
    ...
    for(Robot_Sample_List::iterator it = robot_models.begin(); it != robot_models.end(); it++)
    {
        ...
        draw_robot(it->pos.x, it->pos.y, SSLRefbox::Colors::RED, it->team, it->id, last_touched);
...
void GLExtra::draw_robot(int x, int y, SSLRefbox::Colors::Color color, int team, int id, bool last_touched)
{
    glPushMatrix();
    glTranslatef(x, y, 0.);
    ...
    switch(color)
    {
        ...
        default: //black: default, should not happen
            std::cout << "unknown robot at (" << x << "|" << y << ") color: " << color << std::endl;
            glColor3d(0.,0.,0.);
            glBegin(GL_TRIANGLE_FAN);
                glVertex3d(0.,0.,0.);
                for (int i=0; i <= 12; ++i)
    ...
    if(last_touched)
    {
        ...
        glBegin(GL_QUADS); // Draw a square
            glVertex3f(-BSmart::Field::robot_radius/2, BSmart::Field::robot_radius/2, 0.0f); // oben links
            glVertex3f( BSmart::Field::robot_radius/2, BSmart::Field::robot_radius/2, 0.0f); // oben rechts
            glVertex3f( BSmart::Field::robot_radius/2,-BSmart::Field::robot_radius/2, 0.0f); // unten rechts
            glVertex3f(-BSmart::Field::robot_radius/2,-BSmart::Field::robot_radius/2, 0.0f); // unten links
        glEnd();
    }
...
void GLExtra::draw_ball(double x, double y, double z, SSLRefbox::Colors::Color color)
{
    ...
    switch(color)
    {
        ...
        default: //default, should not happen
            std::cout << "unknown ball at (" << x << "|" << y << ") color: " << color << std::endl;
            ...
            if((it->team == brit->rule_breaker.x) && (it->id == brit->rule_breaker.y))
            {
                glPushMatrix();
                    ...
                    glBegin(GL_LINE_STRIP);
                        for (int i=0; i <= 24; ++i)
                        {
                            glVertex3d(cos(f)*(BSmart::Field::robot_radius+100.),sin(f)*(BSmart::Field::robot_radius+100.),0);
                            f = f + (2*BSmart::pi/24);
                        }
                    glEnd();
                glPopMatrix();
