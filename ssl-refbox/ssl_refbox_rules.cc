...
SSL_Refbox_Rules::SSL_Refbox_Rules(QWaitCondition* rules_wait_condition_, Filter_Data* filter_data_, BSmart::Game_States* gamestate_)
{
//    argv_tmp = argv;
...
{
    /* Construction de l'argument permettant d'initialiser le Prolog */

    char* program = argv_global;
//    char *program = "./bin/ssl-refbox";
    char *plav[2];
    plav[0] = program;
    plav[1] = NULL;
    ...
    PL_put_integer(field_width, BSmart::Field::field_width );
    PL_put_integer(field_height, BSmart::Field::field_height );
    PL_put_integer(goal_width, BSmart::Field::goal_width );
    PL_put_integer(goal_depth, BSmart::Field::goal_depth );
    PL_put_integer(goal_height, BSmart::Field::goal_height );
    PL_put_integer(defense_radius, BSmart::Field::defense_radius );
    PL_put_integer(defense_line, BSmart::Field::defense_line );
    PL_put_integer(penalty_mark, BSmart::Field::penalty_mark_distance );
    ...
    //Spielinitialisierung, Anlegen von Variablen
    predicate_t game_init = PL_predicate("game_init", 0, "Spielinitialisierung");
    ...
    predicate_t set_ball_stuff = PL_predicate("set_ball_stuff", 9 , "set_ball_variable");
    ...
    predicate_t set_timestamp = PL_predicate("set_timestamp", 1, "set_timestamp_variable");

    //Zeiten: Timeout commands
    predicate_t start_yellow_timeout = PL_predicate("start_yellow_timeout", 0, "start yellow timeout");
    predicate_t end_yellow_timeout = PL_predicate("end_yellow_timeout", 0, "end yellow timeout");
    predicate_t start_blue_timeout = PL_predicate("start_blue_timeout", 0, "start blue timeout");
    predicate_t end_blue_timeout = PL_predicate("end_blue_timeout", 0, "end blue timeout");

    //Angaben zur Halbzeit
    predicate_t end_first_half = PL_predicate("end_first_half", 0, "end first half");

    //gamestate
    term_t global_play_state = PL_new_term_refs(1);
    BSmart::Game_States::Play_State play_state_tmp;
    predicate_t set_global_play_state = PL_predicate("set_global_play_state", 1, "set_global_playstate_variable");
    predicate_t set_local_play_state_from_outside = PL_predicate("set_local_play_state_from_outside", 1, "set_local_playstate_variable");
    char refbox_cmd;

    //Spielstart, Sprung in running
    predicate_t game_control = PL_predicate("game_control", 0, "zentrale Spielsteuerung");

    //Aufruf der Regeln
    predicate_t check_rules = PL_predicate("check_rules", 1, "Regelüberprüfung");

    //rule_zero: Nur Regeln checken falls interner und externer Status gleich
    predicate_t rule_zero = PL_predicate("rule_zero", 0, "Spielzustaende gleich");
    ...
    predicate_t get_standing = PL_predicate("get_standing", 2, "Ergebnis");

    //Zeichnungen der Regelverstöße für GUI
    ...
    for(;;)
    {
        rules_mutex.lock();
            ...
            PL_put_integer(timestamp, cur_timestamp);
            ...
            PL_put_integer(global_play_state, play_state_tmp);
            PL_call_predicate(NULL, PL_Q_NORMAL, set_global_play_state, global_play_state);
            if(play_state_tmp != play_state_old)
            {
                play_state_old = play_state_tmp;
                PL_call_predicate(NULL, PL_Q_NORMAL, set_local_play_state_from_outside, global_play_state);
            ...
            PL_put_integer(ball_pos_x, ball_model.pos.x);
            PL_put_integer(ball_pos_y, ball_model.pos.y);
            PL_put_integer(ball_pos_z, ball_model.pos.z);
            PL_put_integer(ball_speed_x, ball_model.speed.x);
            PL_put_integer(ball_speed_y, ball_model.speed.y);
            PL_put_integer(ball_speed_z, ball_model.speed.z);
            PL_put_integer(ball_last_touched_team, ball_model.last_touched_robot.x);
            PL_put_integer(ball_last_touched_id, ball_model.last_touched_robot.y);
            PL_put_integer(ball_status, ball_model.status);
            ...
            for(int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team)
            {
                for(int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id )
                {
                    ...
                    PL_put_integer(robot_team, team);
                    PL_put_integer(robot_id, id);
                    PL_put_integer(robot_pos_x, robot_model.pos.x);
                    PL_put_integer(robot_pos_y, robot_model.pos.y);
                    PL_put_integer(robot_speed_x, robot_model.speed.x);
                    PL_put_integer(robot_speed_y, robot_model.speed.y);
                    PL_put_integer(robot_seen, (int)robot_seen_tmp);
            ...
            for(std::vector<Broken_Rule>::iterator brit = broken_rule_vector.begin(); brit != broken_rule_vector.end();)
            {
                ...
                {
                    std::cout << cur_timestamp << " - " << brit->when_broken << " Regel " << brit->rule_number << " entfernt" << std::endl;
            ...
            //Nur Regeln checken falls interner und externer Status gleich
            if(PL_call_predicate(NULL, PL_Q_NORMAL, rule_zero, NULL))
            {
                ...
                if(PL_call_predicate(NULL, PL_Q_NORMAL, check_rules, broken_rule))
                {
                    ...
                    if(rule != last_break || (cur_frm - last_msg) > 100 )
                    {
                        ...
                        last_break = rule;
                        last_msg = cur_frm;
                        std::cout << cur_frm << " Rule " << rule << " broken by " << team << " | " << id << std::endl;
                    }
                    ...
                    if(rule != -42)
                    {
                        ...
                        PL_call_predicate(NULL, PL_Q_NORMAL, get_freekick_pos, freekick_pos_x);
                        PL_call_predicate(NULL, PL_Q_NORMAL, get_left, left_team);
                        ...
                        switch(rule)
                        {
                            ...
                            case 15:
                                broken_rule_gui.rule_breaker = rule_breaker;
                                broken_rule_gui.circle_around_ball = true;
                                broken_rule_gui.defense_area = (left == rule_breaker.x) ? 1 : 0 ;
                                break;
                            case 16:
                                broken_rule_gui.rule_breaker = rule_breaker;
                                broken_rule_gui.circle_around_ball = true;
                            ...
                            case 17:
                                broken_rule_gui.rule_breaker = rule_breaker;
                                broken_rule_gui.circle_around_ball = true;
                                if( (local_play_state_test_gui < 9 && left == 1) || (local_play_state_test_gui > 8 && left == 0) )//links
                                {
                                    broken_rule_gui.line_for_smth = BSmart::Line(-2170.,-2015.,-2170.,2015.);
                                }
                                else//right
                                {
                                    broken_rule_gui.line_for_smth = BSmart::Line(2170.,-2015.,2170.,2015.);
                            ...
                            case 29:
                                broken_rule_gui.freekick_pos = freekick_pos;
                                broken_rule_gui.rule_breaker = rule_breaker;
                                broken_rule_gui.standing = BSmart::Int_Vector(standing_yellow,standing_blue);
                                break;
                            case 30:
                                broken_rule_gui.rule_breaker = rule_breaker;
                                broken_rule_gui.freekick_pos = freekick_pos;
                            ...
                            case 42:
                                broken_rule_gui.rule_breaker = rule_breaker;
                                broken_rule_gui.freekick_pos = freekick_pos;
                                broken_rule_gui.line_for_smth = BSmart::Line(freekick_pos.x,-2015.,freekick_pos.x,2015.);
                                break;
                            default:
                                ;
                        }
            ...
            for(std::vector<Broken_Rule>::iterator brit = broken_rule_vector.begin(); brit != broken_rule_vector.end(); ++brit)
            {
