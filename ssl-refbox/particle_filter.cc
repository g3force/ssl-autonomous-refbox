...
Particle_Filter_Mother::Particle_Filter_Mother(Pre_Filter_Data* pf_data_, Filter_Data* filter_data_, QWaitCondition* rules_wait_condition_, QWaitCondition* new_data_wait_condition_)
...
{
    connect(this->pf, SIGNAL(change_ball_status_intern(QString)), this, SIGNAL(change_ball_status(QString)));
    connect(this->pf, SIGNAL(change_ball_last_touched_intern(QString)), this, SIGNAL(change_ball_last_touched(QString)));
}
...
void Particle_Filter_Mother::run()
{
    ...
    for(;;)
    {
        //wird aufgeweckt von sslvision.cc execute
            ...
            if(filter_data->get_robot_seen(team, id) && num_robots > 0)
            {
                ...
                for(int i = 0; i < Filter_Data::ROBOT_SAMPLES; ++i)
                {
                    ...
                    if(random < augment) // insert new samples
                    {
                        ...
                        augment_robot.pos = BSmart::Pose(robots[robot_percept].x, robots[robot_percept].y, robots[robot_percept].rotation);
                        //Heuristik
...
void Particle_Filter::determine_ball_status(const Ball_Sample& new_ball_model)
{
    ...
    double angle_to = acos( ( (ball_direction_before.x*ball_direction_after.x + ball_direction_before.y*ball_direction_after.y) / (ball_direction_before.abs() * ball_direction_after.abs() ) ) ) / BSmart::pi * 180;
        ...
        for(Robot_Sample_List::const_iterator it = robot_models.begin(); it != robot_models.end(); it++)
        {
            ...
            if(test_intersection(circle, ball_line))
            {
                if(intersection_point(circle, ball_line, intersections))
                {
                    ...
                    {
                        if(new_ball_model.pos.distance_to_2D(intersections[j]) < new_ball_model.pos.distance_to_2D(it->pos))
                        {
                            BSmart::Pose dir_intersect_to_new_ball_pos = BSmart::Pose(new_ball_model.pos.x - intersections[j].x, new_ball_model.pos.y - intersections[j].y);
                            double collision_angle = acos( ( (dir_intersect_to_new_ball_pos.x*ball_direction_after.x + dir_intersect_to_new_ball_pos.y*ball_direction_after.y) / (dir_intersect_to_new_ball_pos.abs() * ball_direction_after.abs() ) ) ) / BSmart::pi * 180;
    ...
    if((ball_lying_counter == -1) && !intersect)
    {
        ...
        for(Robot_Sample_List::const_iterator it = robot_models.begin(); it != robot_models.end(); it++)
        {
            if(last_ball_model.pos.distance_to_2D(it->pos) < (BSmart::Field::robot_radius + BSmart::Field::ball_radius + 50))
            {
                last_touched_dist_tmp = new_ball_model.pos.distance_to_2D(it->pos) - BSmart::Field::robot_radius - BSmart::Field::ball_radius;
                intersect = true;
                last_contact.frame = newest_frame;
                last_contact.robot.x = it->team;
                last_contact.robot.y = it->id;
    ...
    if( ( fabs(speed_diff_percept) > 1 || fabs(speed_diff_model) > 1 || angle_to > 10) && (ball_lying_counter <= 0) && !intersect)
    {
        last_touched_dist_tmp = std::numeric_limits<double>::max();
        for(Robot_Sample_List::const_iterator it = robot_models.begin(); it != robot_models.end(); it++)
        {
            if(new_ball_model.pos.distance_to_2D(it->pos) < (BSmart::Field::robot_radius + BSmart::Field::ball_radius + 100) )
            {
                circle.x = it->pos.x;
                circle.y = it->pos.y;
    ...
    //ab nem winkel von 60° garantiert kollision
    if(angle_to > 60 && !intersect && !ball_lying_counter)
    {
        last_touched_dist_tmp = std::numeric_limits<double>::max();
        for(Robot_Sample_List::const_iterator it = robot_models.begin(); it != robot_models.end(); it++)
        {
            if(new_ball_model.pos.distance_to_2D(it->pos) < (BSmart::Field::robot_radius + BSmart::Field::ball_radius + 40) )
            {
                intersect = true;
                last_contact.frame = newest_frame;
                last_contact.robot.x = it->team;
    ...
    //schreiben der lc ins log
    for(std::vector<Last_Contact>::iterator lc_it = last_contacts.begin(); lc_it != last_contacts.end();)
    {
        if((timestamp - lc_it->timestamp) > 1000)
        {
            if(log_size == 0 || lc_it->robot != all_contacts_log[log_size-1].robot || (lc_it->timestamp - all_contacts_log[log_size-1].timestamp) > 1000 )
            {
...
{
    //Mathematik für Informatiker Seite 437
