...
Particle_Filter_Mother::Particle_Filter_Mother(Pre_Filter_Data* pf_data_,
                Filter_Data* filter_data_, QWaitCondition* rules_wait_condition_,
...
Particle_Filter::Particle_Filter(Pre_Filter_Data* pf_data_,
                                 Filter_Data* filter_data_) :
...
void Particle_Filter::sensor_update()
{
        ...
        Robot_Percept_List
        cur_robots[Filter_Data::NUMBER_OF_TEAMS][Filter_Data::NUMBER_OF_IDS];
        for (int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team) {
                for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
        ...
        for (int cam = 0; cam < 2; ++cam) {
                ...
                for (int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team) {
                        for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
                                ...
                                std::vector<Robot_Percept>::iterator iter_robot =
                                        robots.begin();
                                std::vector<Robot_Percept>::iterator iter_robot_end =
                                        robots.end();
        ...
        for (int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team) {
                for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
                        filter_data->set_current_robot_percepts(team, id,
                                                                cur_robots[team][id]);
                }
        }
}
...
bool Particle_Filter::weight_ball(Ball_Percept& perc)
{
        ...
        for (Ball_Sample_List::iterator it = samples.begin(); it != samples.end(); it++) {
                if (std::isnan(it->pos.x)) {
                        std::cout << "weight_ball isnan: it->pos.x: " << it->pos.x
                                  << std::endl;
                }
...
void Particle_Filter::resample()
{
        ...
        if (num_balls > 0) {
                ...
                total_weight = 0.;
                best_weight = 0.;
                for (Ball_Sample_List::iterator it = ball_samples_old.begin(); it
                     != ball_samples_old.end(); ++it) {
                ...
                if (std::isnan(total_weight)) {
                        total_weight = 0;
                        std::cout << "total_weight is not a number - please check!"
                                  << std::endl;
                }
                ...
                BSmart::Pose3D ball_speed(pf_data->get_ball_direction_after().x,
                                          pf_data->get_ball_direction_after().y, 0.);
                ...
                for (int i = 0; i < Filter_Data::BALL_SAMPLES; ++i) {
                        ...
                        if (random < augment) { // insert new samples
                                ...
                                new_ball.pos = BSmart::Pose3D(balls[ball_percept].x,
                                                              balls[ball_percept].y, 0.);
                                ...
                                random_derivation = (double) rand() / (double) RAND_MAX
                                                    * total_weight;
        ...
        for (int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team) {
                for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
                        Robot_Percept_List robots =
                                filter_data->get_current_robot_percepts(team, id);
                        ...
                        if (filter_data->get_robot_seen(team, id) && num_robots > 0) {
                                ...
                                //calc total weight
                                total_weight = 0.;
                                for (Robot_Sample_List::iterator it = robot_samples_old.begin(); it
                                     != robot_samples_old.end(); ++it) {
                                        total_weight += it->weighting;
                                        it->age++;
                                }
                                ...
                                o_slow_robots[team][id] += alpha_slow_robots * (average_weight
                                                           - o_slow_robots[team][id]);
                                if (o_slow_robots[team][id] < 0.00000000001)
                                        o_slow_robots[team][id] = 0.00000000001;
                                o_fast_robots[team][id] += alpha_fast_robots * (average_weight
                                                           - o_fast_robots[team][id]);
                                ...
                                for (int i = 0; i < Filter_Data::ROBOT_SAMPLES; ++i) {
                                        ...
                                        if (random < augment) { // insert new samples
                                                ...
                                                augment_robot.pos = BSmart::Pose(
                                                                            robots[robot_percept].x,
                                                                            robots[robot_percept].y,
...
void Particle_Filter::create_models()
{
        ...
        for (Ball_Sample_List::iterator it = ball_samples.begin(); it
             != ball_samples.end(); ++it) {
                ...
                for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
                        ...
                        if (filter_data->get_robot_seen(team, id)) {
                                Robot_Sample_List robot_samples =
                                        filter_data->get_robot_samples(team, id);
...
void Particle_Filter::determine_ball_status(const Ball_Sample& new_ball_model)
{
        ...
        double speed_diff_model = new_ball_model.speed.abs()
                                  - last_ball_model.speed.abs();
        double angle_to =
                acos(
                        ((ball_direction_before.x * ball_direction_after.x
                          + ball_direction_before.y * ball_direction_after.y)
                         / (ball_direction_before.abs()
                            * ball_direction_after.abs())))
                / BSmart::pi * 180;
        ...
        if ((speed_before <= 0.05 && speed_after <= 0.05)
            || (new_ball_model.pos.distance_to_3D(last_ball_model.pos) < 5
                && ball_lying_counter > 4)) {
        ...
        //Ball shot
        if ((speed_diff_percept > 1. || speed_diff_model > 1.)
            && (ball_lying_counter <= 0)) {
                last_touched_dist_tmp = std::numeric_limits<double>::max();
                for (Robot_Sample_List::const_iterator it = robot_models.begin(); it
                     != robot_models.end(); it++) {
                        circle.x = it->pos.x;
                        circle.y = it->pos.y;
                                        ...
                                        for (unsigned int j = 0; j < intersections.size(); ++j) {
                                                if (new_ball_model.pos.distance_to_2D(intersections[j])
                                                    < new_ball_model.pos.distance_to_2D(it->pos)) {
                                                        BSmart::Pose dir_intersect_to_new_ball_pos =
                                                                BSmart::Pose(
                                                                        new_ball_model.pos.x
                                                                        - intersections[j].x,
                                                                        new_ball_model.pos.y
                                                                        - intersections[j].y);
                                                        double
                                                        collision_angle =
                                                                acos(
                                                                        ((dir_intersect_to_new_ball_pos.x
                                                                          * ball_direction_after.x
                                                                          + dir_intersect_to_new_ball_pos.y
                                                                          * ball_direction_after.y)
                                                                         / (dir_intersect_to_new_ball_pos.abs()
                                                                            * ball_direction_after.abs())))
                                                                / BSmart::pi * 180;
                                                        if (collision_angle < 1.) {
                                                                if (new_ball_model.pos.distance_to_2D(
                                                                            intersections[j])
                                                                    < last_touched_dist_tmp) {
                                                                        last_touched_dist_tmp
                                                                        = new_ball_model.pos.distance_to_2D(
                                                                                  intersections[j]);
                                                                        intersect = true;
                ...
                for (Robot_Sample_List::const_iterator it = robot_models.begin(); it
                     != robot_models.end(); it++) {
                        if (last_ball_model.pos.distance_to_2D(it->pos)
                            < (BSmart::Field::robot_radius + BSmart::Field::ball_radius
                               + 50)) {
                                last_touched_dist_tmp = new_ball_model.pos.distance_to_2D(
                                                                it->pos) - BSmart::Field::robot_radius
        ...
        if ((fabs(speed_diff_percept) > 1 || fabs(speed_diff_model) > 1 || angle_to
             > 10) && (ball_lying_counter <= 0) && !intersect) {
                last_touched_dist_tmp = std::numeric_limits<double>::max();
                for (Robot_Sample_List::const_iterator it = robot_models.begin(); it
                     != robot_models.end(); it++) {
                        if (new_ball_model.pos.distance_to_2D(it->pos)
                            < (BSmart::Field::robot_radius + BSmart::Field::ball_radius
                               + 100)) {
                                ...
                                ball_line.p2.x = last_ball_model.pos.x
                                                 + ball_direction_before.x; //2d
                                ball_line.p2.y = last_ball_model.pos.y
                                                 + ball_direction_before.y; //2d
                                intersections.clear();
                                if (test_intersection(circle, ball_line)) {
                                        if (intersection_point(circle, ball_line, intersections)) {
                                                for (unsigned int j = 0; j < intersections.size(); ++j) {
                                                        if (new_ball_model.pos.distance_to_2D(
                                                                    intersections[j]) < last_touched_dist_tmp) {
                                                                last_touched_dist_tmp
                                                                = new_ball_model.pos.distance_to_2D(
                                                                          intersections[j]);
        ...
        for (std::vector<Last_Contact>::iterator fr_it = flying_robots.begin(); fr_it
             != flying_robots.end();) {
                if ((timestamp - fr_it->timestamp) > 1000) {
                        fr_it = flying_robots.erase(fr_it);
                } else {
                        for (std::vector<Last_Contact>::iterator lc_it =
                                     last_contacts.begin(); lc_it != last_contacts.end();) {
                                if (lc_it->robot == fr_it->robot && lc_it->type != 4
                                    && !lc_it->lying_shot) {
        ...
        // write lc to log
        for (std::vector<Last_Contact>::iterator lc_it = last_contacts.begin(); lc_it
             != last_contacts.end();) {
                if ((timestamp - lc_it->timestamp) > 1000) {
                        if (log_size == 0 || lc_it->robot
                            != all_contacts_log[log_size - 1].robot
                            || (lc_it->timestamp
                                - all_contacts_log[log_size - 1].timestamp) > 1000) {
                                all_contacts_log.push_back(*lc_it);
                                log_size++;
                        } else if (log_size != 0 || lc_it->robot
                                   == all_contacts_log[log_size - 1].robot) {
        ...
        if (intersect || flying) {
                ...
                } else if (log_size > 0 && last_touched_robot_saved
                           != all_contacts_log[log_size - 1].robot) {
        ...
        if (new_ball_last_touched != -1 && new_ball_last_touched
            != ball_last_touched_saved) {
                ball_last_touched_saved = new_ball_last_touched;
                emit change_ball_last_touched_intern(
                        Sample::last_touched_names[ball_last_touched_saved]);
        }
...
BSmart::Pose3D Particle_Filter::get_random_pos_3d(int min_x, int max_x,
                int min_y, int max_y, int min_z, int max_z)
{
        return BSmart::Pose3D(random_number(min_x, max_x),
                              random_number(min_y, max_y), random_number(min_z, max_z));
}
...
BSmart::Pose Particle_Filter::get_random_pos_2d(int min_x, int max_x,
                int min_y, int max_y)
{
        return BSmart::Pose(random_number(min_x, max_x),
                            random_number(min_y, max_y));
}

//calculates the dist between a sample in 3D and a percept in 2D
double Particle_Filter::dist_in_2d(int camID, BSmart::Pose3D sample,
                                   BSmart::Pose percept)
{
        ...
        BSmart::Pose pose_2d(sample.x + factor * direction.x,
                             sample.y + factor * direction.y);
...
double Particle_Filter::gaussian(double v, double std_dev)
{
        ...
        return (1.0 / (std_dev * sqrt(2 * BSmart::pi))) * exp(
                       -0.5 * pow(v / std_dev, 2));
}
