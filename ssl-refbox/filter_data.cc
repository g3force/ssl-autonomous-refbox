...
void Filter_Data::set_current_ball_percepts(const Ball_Percept_List& ball_percepts)
{
    samples_mutex.lock();
...
void Filter_Data::set_robot_samples(int team, int id, const Robot_Sample_List& robots)
{
...
void Filter_Data::set_current_robot_percepts(int team, int id, const Robot_Percept_List& cur_robots)
{
    assert(team >= 0 && team < NUMBER_OF_TEAMS);
    assert(id >= 0 && id < NUMBER_OF_IDS);
...
void Filter_Data::set_internal_play_states(BSmart::Int_Vector internal_play_states_)
{
    samples_mutex.lock();
