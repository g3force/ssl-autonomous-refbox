...
void SSLVision::run()
{
        ...
        while (1) {
                ...
                if (queue_filled) {
                        ...
                        if (tf_percept_queue_one_ball[transformed_percept.cam_id].size()
                            > 0) {
                                    ...
                                    == tf_percept_queue_one_ball[transformed_percept.cam_id][0].current_frame) {
                                        ...
                                        if (tf_percept_queue_one_ball[transformed_percept.cam_id].size()
                                            > 0) {
                                                ...
                                                int
                                                timediff =
                                                        tf_percept_queue_one_ball[transformed_percept.cam_id][index].frame_received
                        ...
                        for (int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team) {
                                for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
                                        if (tf_percept_queue_one_robot[transformed_percept.cam_id][team][id].size()
                                            > 0) {
                        ...
                        if (transformed_percept.balls.size() == 0) {
                                data->set_ball_framenumber(transformed_percept.cam_id,
                                                           transformed_percept.ball_frame_number);
                                data->set_ball_timestamp(transformed_percept.cam_id,
                                                         transformed_percept.frame_received);
                        } else {
                                data->clear_balls(transformed_percept.cam_id);
                                data->set_balls(transformed_percept.cam_id,
                                                transformed_percept.balls);
                        }

                        if (transformed_percept.has_one_ball) {
                                data->set_ball_direction_before(
                                        transformed_percept.ball_direction_before);
                                data->set_ball_direction_after(
                                        transformed_percept.ball_direction_after);
                        }

                        for (int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team) {
                                for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
                                        data->set_robots(transformed_percept.cam_id, team, id,
                                                         transformed_percept.robots[team][id]);
                                        if (transformed_percept.has_one_robot[team][id]) {
                                                data->set_robot_direction(team, id,
                                                                          transformed_percept.robot_direction[team][id]);
                                        }
                                }
                        }
...
int SSLVision::execute(Transformed_Percept& trans_perc)
{
        ...
        if (play) {
                ...
                if (!(log_control->get_prop_next_frame() < 0)) {
                        ...
                        double
                        new_time =
                                logs.log(log_control->get_prop_next_frame()).frame().t_capture();
                        double timediff = new_time - old_time;
                        has_new_frame = true;
                        if (log_control->get_play_speed() != 0) {
                                time_diff = abs(
                                                    ((timediff * 1000) / log_control->get_play_speed()));
                        } else {
...
int SSLVision::recv(SSL_DetectionFrame& frame)
{
        ...
        if (packet.has_geometry()) {
                ...
                for (int i = 0; i < n_cams; ++i) {
                        ...
                        BSmart::Pose3D cam_pos(cam.derived_camera_world_tx(),
                                               cam.derived_camera_world_ty(),
...
void SSLVision::analyse_percepts()
{
        ...
        if (transformed_percept.balls.size() == 1) {
                ...
                int size_one_ball =
                        tf_percept_queue_one_ball[transformed_percept.cam_id].size();

                if (size_one_ball == 1) {
                        transformed_percept.ball_direction_before
                        = (transformed_percept.balls[0]
                           - tf_percept_queue_one_ball[transformed_percept.cam_id][size_one_ball
                                           - 1].balls[0]);
                        int
                        timediff =
                                (transformed_percept.frame_received
                        ...
                        if (timediff == 0) {
                                transformed_percept.ball_direction_before
                                = tf_percept_queue_one_ball[transformed_percept.cam_id][size_one_ball
                                                - diff].ball_direction_before;
                        } else {
                                transformed_percept.ball_direction_before /= timediff;
                        }

                }
                tf_percept_queue_one_ball[transformed_percept.cam_id].push_back(
                        transformed_percept);
        }
        ...
        for (int team = 0; team < Filter_Data::NUMBER_OF_TEAMS; ++team) {
                for (int id = 0; id < Filter_Data::NUMBER_OF_IDS; ++id) {
                        if (transformed_percept.robots[team][id].size() == 1) {
                                ...
                                int
                                size_one_robot =
                                        tf_percept_queue_one_robot[transformed_percept.cam_id][team][id].size();
                                if (size_one_robot > 0) {
                                        transformed_percept.robot_direction[team][id]
                                        = (transformed_percept.robots[team][id][0]
                                        ...
                                        if (timediff == 0) {
                                                transformed_percept.robot_direction[team][id]
                                                = tf_percept_queue_one_robot[transformed_percept.cam_id][team][id][size_one_robot
                                                                - 1].robot_direction[team][id];
                                        } else {
                                                transformed_percept.robot_direction[team][id]
                                                /= timediff;
                                        }
                                }
                                tf_percept_queue_one_robot[transformed_percept.cam_id][team][id].push_back(
                                        transformed_percept);
                        }
                }
        }
...
int SSLVision::end_record()
{
        ...
        fileName = QFileDialog::getSaveFileName((QWidget*) this->parent(),
                                                tr("Save Logfile"), fileName, tr("Log Files (*.log)"));
        ...
        std::fstream output(fileName.toAscii().constData(),
